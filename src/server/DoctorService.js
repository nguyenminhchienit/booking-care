...
let getListPatientForDoctorDoneService = (doctorId, date, status) => {
  return new Promise(async (resolve, reject) => {
    try {
      if (!doctorId || !date) {
        resolve({
        ...
        resolve({
          errCode: 0,
          message: "Get list patient done for doctor succeed",
          data: data,
        });
      }
    } catch (e) {
...
let postHandleSendRemedyService = (data) => {
  return new Promise(async (resolve, reject) => {
    try {
...
let postHandleSendRemedyDoneService = (data) => {
  return new Promise(async (resolve, reject) => {
    try {
      if (!data.doctorId || !data.patientId || !data.timeType || !data.email) {
        resolve({
        ...
        //Send email
        await EmailService.sendExtra(data);
...
module.exports = {
  postHandleSendRemedyDoneService,
  getDoctor,
  getAllDoctor,
