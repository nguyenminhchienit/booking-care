...
let getScheduleDoctor = async (req, res) => {
  try {
    let info = await DoctorService.getScheduleDoctorService(
      req.query.date,
      req.query.doctorId
    );
    return res.status(200).json(info);
...
let getExtraDoctorInfo = async (req, res) => {
  try {
    let info = await DoctorService.getExtraDoctorInfoService(
      req.query.doctorId
    );
    return res.status(200).json(info);
  } catch (e) {
...
let getListPatientForDoctorDone = async (req, res) => {
  try {
    let info = await DoctorService.getListPatientForDoctorDoneService(
      req.query.doctorId,
      req.query.date,
      "S3"
    );
...
let postHandleSendRemedyDone = async (req, res) => {
  try {
    let info = await DoctorService.postHandleSendRemedyDoneService(req.body);
    return res.status(200).json(info);
  } catch (e) {
    console.log(e);
    return res.status(200).json({
...
module.exports = {
  ...
  getListPatientForDoctorDone,
  postHandleSendRemedyDone,
};
